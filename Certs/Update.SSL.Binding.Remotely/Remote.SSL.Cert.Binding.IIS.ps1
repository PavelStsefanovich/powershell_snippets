...
foreach ($nodename in $nodesToUpdate.GetEnumerator().name) {
    ...
    if ($nodesToUpdate.$nodename.isReachable) {
        try {
            ...
            Invoke-Command -ComputerName $computerName -FilePath "$WORKSPACE\SSL.Cert.Binding.IIS.ps1" -ArgumentList $certFilename,$CertPassw,$CaCertFilename,$DomainName,"c:\BR" -ErrorAction stop
