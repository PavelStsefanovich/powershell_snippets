[CmdletBinding(HelpUri = "https://ewiki.athoc.com/display/BR/SSL.Cert.Binding.IIS.ps1")]
Param (
...
try {
    ...
    if (Test-Path 'C:\inetpub\wwwroot') {
        $siteFolder = Join-Path -Path 'C:\inetpub\wwwroot' -ChildPath $Website
    } else {
    ...
    foreach ($cert in (ls Cert:\LocalMachine -Recurse | ?{$_.Subject -like "*$DomainName*"})) {
        rm ("cert:\" + (convert-path $cert.PSPath -ErrorAction SilentlyContinue)) -Force
    }
