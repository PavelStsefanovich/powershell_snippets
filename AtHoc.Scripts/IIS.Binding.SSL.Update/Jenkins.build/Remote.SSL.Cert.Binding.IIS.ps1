[CmdletBinding(HelpUri = "https://ewiki.athoc.com/display/BR/Remote.SSL.Cert.Binding.IIS.ps1")]
Param(
    [string]$Certfile,
    [string]$CertPassw,
    [string]$CaCertFile,
...
foreach ($nodename in $nodesToUpdate.GetEnumerator().name) {
    ...
    if ($nodesToUpdate.$nodename.isReachable) {
        try {
            ...
            Invoke-Command -ComputerName $computerName -FilePath "$WORKSPACE\SSL.Cert.Binding.IIS.ps1" -ArgumentList $certFilename,$CertPassw,$CaCertFilename,$DomainName,"c:\BR",$Website -ErrorAction stop
